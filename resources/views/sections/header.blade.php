                ...
                <div class="row ">
                    ...
                    <div class="col-md-12 mb-3 mobSearch hide_desktop input-group">
                        ...
                        <form id="searchFormMobile" action="{{ route('front.search') }}" method="GET" class="w-100 dif">
                            ...
                            <input type="hidden" name="globalSearchMobileLoc" id="globalSearchMobileLoc" class=" form-control" value="">
                                    ...
                                    <div class="row">
                                        <ul class="nav flex-column">
                                            @foreach ($headerCategories as $category)
                                                <li class="nav-item">
                                                <a class="nav-link active" href="{{ url('services/'.$category->slug) }}"><span></span>{{ $category->name }}</a>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
...
@push('footer-script')
    ...
    <script>
        ...
        function getSpotlight(spotlight, text = "{{ __('front.addToCart') }}")
        {
            ...
            else
            {
                text = `<a
                            ...
                            data-deal-service-type="${spotlight.deal.deal_service_type}"
                            aria-expanded="false">
                            ${text}
                        </a>`;
            }
        ...
        $(document).ready(function () {
            ...
            let interval = setInterval(function () {
                $('.time-left').each(function () {
                    ...
                    totalSeconds -= 1;
                    totalSeconds = parseInt(totalSeconds) //because moment js dont know to handle number in string format
                    if(totalSeconds <= 0)
                    {
                    ...
                    hours = moment.duration(totalSeconds,'seconds').asHours() >= 10 ? Math.floor(moment.duration(totalSeconds,'seconds').asHours()) : '0'+Math.floor(moment.duration(totalSeconds,'seconds').asHours());

                    minute = moment.duration(totalSeconds,'seconds').minutes() >= 10 ? moment.duration(totalSeconds,'seconds').minutes() : '0'+moment.duration(totalSeconds,'seconds').minutes();

                    second = moment.duration(totalSeconds,'seconds').seconds() >= 10 ? moment.duration(totalSeconds,'seconds').seconds() : '0'+moment.duration(totalSeconds,'seconds').seconds();
                ...
                if (navigator.geolocation) {
                    ...
                    if ( location_id !== '' && location.href == '{{ route('front.index').'/' }}')
                    {
                        ...
                        $.easyAjax({
                            url: url,
                            type: 'GET',
                            data: {location_id : location_id, lat: lat, long:long},
                            blockUI: false,
                            success: function (response)
                            {
                                        ...
                                        response.nearByLocations.forEach(services => {
                                            ...
                                            <div class="nearbyserviceBox">
                                                <a href="{{url('service/`+services.category.name+`/`+services.slug+`/')}} ">
                                                    <img src="{{asset('front/images/pixel.gif')}}" data-src="`+services.service_image_url+`" class="img-fluid" alt="Cafes" />
                                                    </a>
                                                </div>
                                            </div>`
                                        });

                                        let viewAllNearByLocations = response.total_nearByLocations_count > 8 ? `<div class="row justify-content-center mt-3">
                                        <a href="{{ route('front.services', 'all') }}" class="view_all hvr-radial-out">View All</a>
                                        </div>` : '';

                                        nearByLocations = nearByLocations+`</div>`+viewAllNearByLocations+`</div>`;
                                ...
                                if(response.spotlight.length>0)
                                {
                                    ...
                                    let spotlight_status = getSpotlightAsPerSetting(spotlight);
        ...
        $('body').on('click', '.add-to-cart', function ()
        {
            ...
            var data = {id, type, price, name, companyId, deal_service_type, unique_id, max_order, serviceType, _token: $("meta[name='csrf-token']").attr('content')};
        ...
        function setSearchBoxLocation(){
            ...
            $('#globalSearchLoc').val(location);
            $('#globalSearchMobileLoc').val(location);
        }
