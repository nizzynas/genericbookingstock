...
@section('content')
    <div class="row mb-2">
        ...
        @if ($isEmployeesHasLocation)
            <div class="col-md-12">
                <div class="alert alert-warning">
                        <div class="row">
                            <div class="col-md-10 d-flex align-items-center">@lang('messages.updateLocationOfEmployee')
                            </div>
                            <div class="col-md-2 text-right">
                                <a href="{{ route('admin.employee.index') }}" ><button type="button" role="button" class="btn btn-success">@lang('app.update')</button> </a>
                            </div>
                        </div>
                </div>
            </div>
        ...
        @if ($user->roles()->withoutGlobalScopes()->latest()->first()->hasPermission('read_booking') || $user->roles()->withoutGlobalScopes()->latest()->first()->hasPermission('create_booking') || \Session::get('loginRole'))
            <div class="col-md-12">
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            <div class="row">
                ...
                <!-- /.col -->
                <div class="col-md-4 col-sm-6 col-12">
                    <div class="info-box link-stats booking-status" data-booking-status="approved">
                ...
                <!-- /.col -->
                <div class="col-md-4 col-sm-6 col-12">
                    <div class="info-box link-stats booking-status" data-booking-status="inprogress">
                ...
                @if($user->is_admin && $current_emp_role->name == 'administrator')
                    <div class="col-md-4 col-sm-6 col-12">
                        <div class="info-box link-stats booking-status" data-booking-status="walkinbookings">
                    ...
                    <div class="col-md-4 col-sm-6 col-12">
                        <div class="info-box link-stats booking-status" data-booking-status="onlinebookings">
                    ...
                    <div class="col-md-4 col-sm-6 col-12">
                        <div class="info-box link-stats total-customer">
                    ...
                    <!-- /.col -->
                    <div class="col-md-4 col-sm-6 col-12">
                        <div class="info-box link-stats total-earnings">
                ...
                @if($user->is_employee && $current_emp_role->name == 'employee')
                    <div class="col-md-4 col-sm-6 col-12">
                        <div class="info-box link-stats booking-status" data-booking-status="assignedpending">
                            <span class="info-box-icon bg-warning"><i class="fa fa-calendar"></i></span>

                            <div class="info-box-content">
                                <span class="info-box-text assignedPendingBooking" onclick="this.blur()" data-original-title="@lang('modules.dashboard.assignedPendingBookings')">@lang('modules.dashboard.assignedPendingBookings')</span>
                                <span class="info-box-number" id="assigned-pending-booking">0</span>
                            </div>
                            <!-- /.info-box-content -->
                        </div>
                        <!-- /.info-box -->
                                            ...
                                            <td>
                                                @if ($booking->items->count()>0)
                                                    ...
                                                    @foreach($booking->items as $key=>$item)
                                                        ...
                                                        <li>
                                                            {{ucwords($item_name)}} x {{ $item->quantity }}
                                                        </li>
                                                    @endforeach
                                                    </ul>
                                                @else
        ...
        @endif
        <div class="col-md-{{$user->roles()->withoutGlobalScopes()->latest()->first()->hasPermission('read_booking') || $user->roles()->withoutGlobalScopes()->latest()->first()->hasPermission('create_booking') || \Session::get('loginRole') ? '4' : '12'}}">
            <div class="row">
                @if(($user->is_admin || $user->is_employee) && !\Session::get('loginRole') && ($current_emp_role->name == 'employee' || $current_emp_role->name == 'administrator'))
                    <div class="col-md-12" id="todo-items-list"></div>
                @endif
            </div>
        </div>
    </div>
...
@push('footer-js')
    ...
    <script>
            ...
            $.easyAjax({
                ...
                success: function (response) {
                    if (response.status == "success") {
                        ...
                        $('#assigned-pending-booking').html(response.assignedPendingBooking)
        ...
        $('body').on('click', '#view-customization', function() {
            $.easyAjax({
                type: 'GET',
                url: '{{ route("admin.dashboard") }}',
                data: {startDate: startDate, endDate: endDate},
        ...
        $('body').on('click', '.booking-status', function() {

            let bookingStatus = $(this).data('booking-status');
            let startDate = $('#start-date').val();
            let endDate = $('#end-date').val();

            var url = '{{ route("admin.bookings.index") }}?status='+bookingStatus+'&startDate='+startDate+'&endDate='+endDate;
            window.location.href = url;
        });

        $('body').on('click', '.total-customer', function() {
            var url = "{{ route('admin.customers.index') }} ";
            window.location.href = url;
        });

        $('body').on('click', '.total-earnings', function() {
            let startDate = $('#start-date').val();
            let endDate = $('#end-date').val();

            var url = '{{ route("admin.reports.index") }}?startDate='+startDate+'&endDate='+endDate;
            window.location.href = url;
        });
    </script>
@endpush
